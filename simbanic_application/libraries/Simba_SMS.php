...
Class Simba_SMS
{
	...
	{
	    //http://trans.magicsms.co.in/api/v3/index.php?method=sms&api_key=A2a58dc64954c4fb6c539ffc8777beae5&to=9724563175&sender=WALART&message=Congratulation Dr.XXXXX, Your VBM Payment Rs. XXXXX is Generate So, as soon as possible it comes to You. Thank you sir, VBM Family, Walart Parivar.&format=json
	    
		$this->ci =& get_instance();
		$this->ci->config->load('sms');

		$api_url = $this->ci->config->item('api_url', 'sms');
		$method = $this->ci->config->item('method', 'sms');
		$api_key = $this->ci->config->item('api_key', 'sms');
		$sender_id = $this->ci->config->item('sender_id', 'sms');

		$this->setMethod($method);
		$this->setWorkingKey($api_key);
	...
	{
		$this->process_sms($to,$message,$dlr_url,$type="xml",$time="null",$unicode=1);
	}
