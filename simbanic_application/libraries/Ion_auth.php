...
class Ion_auth
{
	...
	public function is_depot($id=false)
	{
		$this->ion_auth_model->trigger_events('is_depot');

		$depot_group = $this->config->item('depot_group', 'ion_auth');

		return $this->in_group($depot_group, $id);
	}

	public function is_customer($id=false)
	{
		$this->ion_auth_model->trigger_events('is_customer');

		$customer_group = $this->config->item('customer_group', 'ion_auth');

		return $this->in_group($customer_group, $id);
	}

	public function is_doctor($id=false)
	{
		$this->ion_auth_model->trigger_events('is_doctor');

		$customer_group = $this->config->item('customer_group', 'ion_auth');

		if($this->in_group($customer_group, $id))
		{
			if($id)
			{
				$user_id = $id;
			...
			$user_info = $this->getSimbanicUser($user_id);
			if(isset($user_info) && isset($user_info->customer_type) && $user_info->customer_type == 'doctor')
			{
				return true;
			}
			else
	...
	public function is_medical_store($id=false)
	{
		$this->ion_auth_model->trigger_events('is_doctor');
		...
		if($this->in_group($customer_group, $id))
		{
			...
			$user_info = $this->getSimbanicUser($user_id);
			if(isset($user_info) && isset($user_info->customer_type) && $user_info->customer_type == 'medical_store')
			{
				return true;
			}
			else
	...
	public function is_pharma($id=false)
	{
		$this->ion_auth_model->trigger_events('is_pharma');

		$pharma_group = $this->config->item('pharma_group', 'ion_auth');

		return $this->in_group($pharma_group, $id);
	}
	...
	public function getSimbanicUser($id)
	{
		return $this->ion_auth_model->simbanicUser($id);
	}
